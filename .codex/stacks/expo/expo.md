...
Korean note: 라이브러리를 추가할 때 Expo Web 지원 여부나 플랫폼별 fallback을 확인한다.
...
Korean note: 프로젝트가 웹을 지원하면 웹 진입점과 주요 흐름도 함께 확인한다.
